...
public class Menu {
	...
	public Menu() {
		...
		if (Menu.rootMenu == null) {
			...
			this.exitItem = new MenuItem("Wyjście z gry."); // A root menu will exit from the program
		}
		else {
			this.setTitle("Sub Menu");
			this.exitItem = new MenuItem("Cofnij."); // A sub menu will go back one level
		}
	 ...
	 */
	public void execute(boolean exitAfter) {
		MenuItem item = null;
		do {
			this.print();
		...
		while(!item.isExitItem() && !exitAfter);
	}
